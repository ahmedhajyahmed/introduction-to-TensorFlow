...
def skipgram():
    ...
    with tf.variable_scope('word2vec') as scope:
        ...
        similarty = tf.matmul(val_embeddings, normalized_embeddings, transpose_b=True)
        return batch_inputs, batch_labels, normalized_embeddings, loss, similarty


def run():
    batch_inputs, batch_labels, normalized_embeddings, loss, similarty = skipgram()
